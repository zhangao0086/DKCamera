...
open class DKCamera: UIViewController {
    ...
    open func setupUI() {
        ...
        let cancelButton: UIButton = {
            let cancelButton = UIButton()
            cancelButton.addTarget(self, action: #selector(DKCamera.dismissAction), for: .touchUpInside)
    ...
    // FAILS in swift3.0 as ambiguous.  It seems there is some other dismiss action somewhere.
                        ...
                        if error == nil {
                            if let imageData = AVCaptureStillImageOutput.jpegStillImageNSDataRepresentation(imageDataSampleBuffer) {
                                ...
                                if let didFinishCapturingImage = self.didFinishCapturingImage, let takenImage = UIImage(data: imageData) {
                                    ...
                                    if let takenCGImage = takenImage.cgImage {
